...
DB = [
    ...
    {"id": "CERT-005", "name": "Петро Щур", "instructor": "Ігор Технік", "date": "2023-02-01"}, # Скоро закінчується (3 роки закінчуються 2025-02-01)
...
cert_id = st.text_input("Номер сертифіката (дивись підказку нижче):").strip().upper()
...
with st.expander("Довідка по тестовим ID"):
    ...
    st.write("CERT-004 - Закінчується скоро (лютий 2025)")
